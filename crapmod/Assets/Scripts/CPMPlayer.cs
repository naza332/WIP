...
public class CPMPlayer : MonoBehaviour
{
   ...
   \*******************************************************************************************************/
    ...
    private void ScrollJump()
    {
        if (holdJumpToBhop)
        {
            wishJumps = (Input.GetAxisRaw("Mouse ScrollWheel") < 0f);
            return;
        }

        if (Input.GetAxisRaw("Mouse ScrollWheel") < 0f && !wishJumps)
            wishJumps = true;
    ...
    */
    ...
    void OnTriggerEnter(Collider other)
    {
       if (other.gameObject.CompareTag("Bounceable"))
        {
            if (_controller.velocity.magnitude > 6f)
            {
                playerVelocity.y = _controller.velocity.magnitude;
            }

        }

        if (other.gameObject.CompareTag("platformjump"))
        {
            playerVelocity.y = 30f;
        }

        if (other.gameObject.CompareTag("speedy"))
        {
            playerVelocity.z = _controller.transform.forward.z * 100f;
        }
    }
}